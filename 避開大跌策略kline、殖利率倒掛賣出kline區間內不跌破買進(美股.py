...
# 讀取CSV
T10Y2Y = pd.read_csv('T10Y2Y(W).csv')
...
# 取得資料
df=yf.download('^GSPC',start='1976-6-7',end='2023-10-01',interval='1wk')
#避免Open出現0值
df['Open'] = df.apply(lambda row: row['Close'] if row['Open'] == 0 else row['Open'], axis=1)
# ta-lib 格式
df2 = df.rename(columns={"High": "high", "Low": "low",})
# #合併資料
T10Y2Y.set_index(pd.DatetimeIndex(T10Y2Y.index), inplace=True)
result = pd.concat([df, T10Y2Y], axis=1, join='inner')
T10Y2Y = result[['T10Y2Y']]
# 將T10Y2Y['T10Y2Y']轉為numeric
T10Y2Y['T10Y2Y'] = pd.to_numeric(T10Y2Y['T10Y2Y'], errors='coerce')
print(result)
def defT10Y2Y(data):  # Data is going to be our OHLCV
    # 取得T10Y2Y值
    return data
...
class MAStra(Strategy):
    ...
    def init(self):
        ...
        self.I(defT10Y2Y,T10Y2Y['T10Y2Y'])
